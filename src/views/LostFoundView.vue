...
onMounted(async () => {
  const res = await api.lostItems.list()
  ...
  <section>
    ...
    <div class="list">
      <div v-for="item in items" :key="item.id" class="card">
        ...
        <p class="meta">{{ item.location }} · {{ item.createTime }}</p>
      </div>
    </div>
  </section>
</template>
...
.tag.失物 { background: #fff0f0; color: #e74c3c; }
.tag.寻物 { background: #fff7e6; color: #e67e22; }
